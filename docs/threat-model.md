...
| Nonce reuse                           | The same AES-GCM nonce is reused with one key                          | Current: browser encryption generates a fresh random nonce for each item encryption, and API envelope validation enforces nonce length. Planned: additional repeated-nonce regression checks |
| Cross-site scripting                  | Injected JavaScript reads an access token or unlocked vault            | Current: refresh tokens remain `HttpOnly`; access tokens and unwrapped vault keys stay in memory only; vault keys are cleared on explicit lock, auth loss, hidden tab, and inactivity. Planned: stricter CSP and continued dependency review |
