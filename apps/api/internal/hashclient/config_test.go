...
func TestNewConfigAcceptsValidConfig(t *testing.T) {
	...
	if config.Address() != "127.0.0.1:50051" {
		t.Fatalf("Address() = %q, want 127.0.0.1:50051", config.Address())
	}

	if config.DialTimeout() != 2*time.Second {
		t.Fatalf("DialTimeout() = %v, want 2s", config.DialTimeout())
	}

	if config.RequestTimeout() != 5*time.Second {
		t.Fatalf("RequestTimeout() = %v, want 5s", config.RequestTimeout())
	}
}

func TestNewConfigAcceptsDockerServiceAddress(t *testing.T) {
	t.Parallel()
	...
	if config.Address() != "hash-service:50051" {
		t.Fatalf("Address() = %q, want hash-service:50051", config.Address())
	}
}
